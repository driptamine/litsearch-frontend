...
function MovieAndPersonAutoSearch({ className, autoFocus }) {
  ...
  const trackIds = useSelector(state => selectors.selectTrackSearchResultIds(state, searchValue)) || [];
  ...
  const artistIds = useSelector(state => selectors.selectArtistSearchResultIds(state, searchValue)) || [];
  const artists = useSelector(state => selectors.selectArtists(state, artistIds));

  const personIds = useSelector(state => selectors.selectPersonSearchResultIds(state, searchValue)) || [];
  const people = useSelector(state => selectors.selectPeople(state, personIds));

  const isFetching = useSelector(state => selectors.selectIsFetchingSearch(state));
  ...
  function handleSelectSuggestion(selectedSuggestion) {
    if (selectedSuggestion) {
      switch (selectedSuggestion.suggestionType) {
        case "movie":
          historyPush(`/search/movie?query=${selectedSuggestion.title}`);
          break;
        case "person":
