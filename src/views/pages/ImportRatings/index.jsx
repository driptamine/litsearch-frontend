...
const ImportRatings = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await axios.post(
        `${LITLOOP_API_URL}/movies/import_ratings_csv/`,
        formData,
        { headers: { ...authHeader(), 'Content-Type': 'multipart/form-data' } }
      );
      setResult(res.data);
      setImported(res.data.ratings_created || 0);
      setStatus('done');
    } catch (err) {
  ...
  return (
    <Container
      ...
      onDrop={handleDrop}
    >
      <Title>Import IMDb Ratings</Title>

      <Section>
        <SectionTitle>Upload your CSV</SectionTitle>
        <Description>
          Export your ratings from IMDb, then upload the CSV file here.
        </Description>

        <FileInputRow>
          <FileInputLabel htmlFor="ratings-csv-file">
        ...
        {totalRows > 0 && (
          <RowCount>{totalRows} ratings found in CSV</RowCount>
        )}

        {status === 'idle' && file && (
          <ImportBtn onClick={handleImport}>
            Import {totalRows} ratings
          </ImportBtn>
        )}
        ...
        {status === 'done' && (
          <ResultSection>
            ...
            <ResultText>
              {result.ratings_created} new ratings
              {result.ratings_updated > 0 && (
                <> &middot; {result.ratings_updated} updated</>
              )}
            </ResultText>
            {result.movies_imported > 0 && (
              <ResultSubtext>
                {result.movies_imported} new movies created from CSV
              </ResultSubtext>
            )}
          </ResultSection>
        )}
