...
import { PhotoCardUnsplash } from 'views/pages/PhotosPage/PhotoCardUnsplash';
import { api_data } from 'views/pages/PhotosPage/data/api_data.jsx';

import { PhotoUploader } from 'views/components/upload/uploader/photos/PhotoUploader'

const videos_data = api_data;


export const PhotosPageUnsplash = () => {
  ...
  return (
    <Container>
      ...
      <Grid>
        {photos.map((photo) => (
          <PhotoCardUnsplash key={photo.id} photo={photo} loading={false} />
        ))}

        {/* New skeletons only for next page */}
        {loading &&
          Array.from({ length: newSkeletonCount }).map((_, i) => (
            <PhotoCardUnsplash key={`skeleton-${i}`} photo={null} loading={true} />
          ))}
      </Grid>
