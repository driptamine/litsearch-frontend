  ...
  return (
    <div className="row d-flex m-0 custom-player-controller">
      <div className="custom-player-controller-play-pause text-center">
        {props.playing && (
          <p onClick={props.handlePause} className="m-0">
            <i className="fa fa-pause text-white" />
          </p>
        )}
        {!props.playing && (
          <p onClick={props.handlePlay} className="m-0">
            <i className="fa fa-play text-white" />
          </p>
        )}
      </div>
      <div
        className="custom-player-controller-volumne row m-0 "
        style={{
          alignItems: "center",
        }}
      >
        <div style={{ alignSelf: "center", margin: "0px 5px", width: "10%" }}>
          ...
          {props.volume <= 50 && props.volume > 0 && (
            <i className="fa fa-volume-down text-white" />
          )}
        </div>
        <input
