...
INSTANCE.interceptors.request.use(
  async (config) => {
    try {
      ...
      const session = await litloopAPI.currentSession();
...
const API = {
  ...
  getGoogleTokens: async (codeFromUrl) => {
    const data = {};

    if (codeFromUrl) data.code = codeFromUrl;

    return await INSTANCE.post(`/google/token`, data)
      .then(async (res) => {
        if (res.data.access_token || res.data.refresh_token) {
          console.log('Google: New Access token fetched');
          addLogBase({
            title: 'Google re-authenticated',
            text: 'Successfully re-authenticated to Google (renewed access token)',
            icon: 'google',
          });
          return {
            access_token: res.data.access_token,
