  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase("user/fetchCurrentUser/succeeded", (state, action) => {
        ...
        const username = userData.username || userData.user?.username;
        const currentUserId = userData.id || userData.userId || userData.pk || state.activeAccountId || `user_${Date.now()}`;
        ...
        // Explicitly set djangoAvatar to distinguish from OAuth profileImg
        state.djangoAvatar = getBestAvatar(userData);
        
        // Update profileImg ONLY if djangoAvatar is present, otherwise keep what we have (e.g. OAuth)
      ...
      .addCase("user/oauth/setUserProfile", (state, action) => {
        ...
        const newAccount = {
          ...
          user__username: response.username, // Attach for compatibility
        ...
        state.user__username = newAccount.username; // Attach for compatibility
